#!/usr/bin/env python3
"""
NEPSE Bollinger Band Breakout Strategy — Rule-Based Backtest
============================================================
Volatility contraction (squeeze) followed by expansion breakout.

Entry Logic (Long):
  1. Bollinger Bandwidth is in the lowest 20% of its 120-bar range   — squeeze
  2. Price closes above Upper Bollinger Band                          — breakout
  3. Volume > 1.5× its 20-period SMA                                 — volume surge
  4. ADX(14) rising (ADX[t] > ADX[t-1])                              — momentum confirm

Entry Execution:
  Buy-stop at the upper band level (confirms breakout holds).
  If not filled within ORDER_TIMEOUT bars, cancel.

Stop / Exit:
  - Initial stop: Middle Band (SMA20) − ATR_STOP_MULT × ATR
  - Trail: Middle Band (ratchets up only)
  - Exit: close below Middle Band
  - Hard stop if price gaps below stop

Lookahead Bias Prevention:
  - All indicators use data up to bar t
  - Signal at close of t → stop order fills on bar t+1
...
BB_PERIOD        = 20
BB_STD           = 2.0
BW_LOOKBACK      = 120         # lookback for bandwidth percentile
BW_SQUEEZE_PCT   = 20          # bandwidth must be in bottom 20% = squeeze
...
# Backtest parameters
MIN_ROWS         = 250
WARMUP           = 140         # 120 + BB_PERIOD buffer
...
def setup():
    ...
    log = logging.getLogger("bollinger_breakout")
    log.setLevel(logging.INFO)
    log.handlers.clear()
    fmt = logging.Formatter("%(asctime)s  %(message)s", datefmt="%H:%M:%S")
    fh = logging.FileHandler(RUN_DIR / "bollinger_breakout.log", encoding="utf-8")
    fh.setFormatter(fmt); log.addHandler(fh)
    sh = logging.StreamHandler(sys.stdout)
    sh.setFormatter(fmt); log.addHandler(sh)

    log.info("=" * 70)
    log.info("NEPSE Bollinger Band Breakout Strategy — Rule-Based Backtest")
    log.info("=" * 70)
    log.info(f"Run directory  : {RUN_DIR.resolve()}")
    log.info(f"BB period/std  : {BB_PERIOD}/{BB_STD}")
    log.info(f"Squeeze pctl   : bottom {BW_SQUEEZE_PCT}% of {BW_LOOKBACK} bars")
    log.info(f"Volume mult    : {VOL_MULT}×")
...
def compute_indicators(df):
    ...
    bandwidth = (upper - lower) / mid.replace(0, 1e-10)  # relative bandwidth

    # Bandwidth percentile (rolling)
    bw_pctile = bandwidth.rolling(BW_LOOKBACK).apply(
    ...
    return {
        ...
        "bw_pctile": bw_pctile.values, "adx": adx.values,
        "atr": atr.values, "vol_ma": vol_ma.values,
    }
...
def backtest_ticker(ticker, df):
    ...
    for t in range(WARMUP, n):
        if any(np.isnan(x[t]) for x in [mid, upper, bw_pct, adx, atr]):
            continue
        ...
        if state == "PENDING":
            ...
            if t - sig_bar >= ORDER_TIMEOUT or c[t] < mid[t]:
        ...
        if state == "POSITION":
            ...
            if o[t] <= stop_px:
                ...
                exit_px = c[t]; exit_rsn = "mid_close"
            else:
                trail = mid[t] - ATR_STOP_MULT * atr[t]
        ...
        # ── FLAT — scan for squeeze breakout ──────────────────────────
        squeeze = bw_pct[t] <= BW_SQUEEZE_PCT           # bandwidth in bottom 20%
        breakout = c[t] > upper[t]                       # close above upper band
        vol_ok = not np.isnan(volma[t]) and volma[t] > 0 and v[t] > VOL_MULT * volma[t]
        adx_rising = t > 0 and adx[t] > adx[t-1]

        if squeeze and breakout and vol_ok and adx_rising:
            pend_level = upper[t]
            pend_stop  = mid[t] - ATR_STOP_MULT * atr[t]
            sig_bar    = t
            direction  = "LONG"
            state      = "PENDING"
...
def report_results(all_trades, log, run_dir):
    ...
    log.info(f"\n{'='*70}")
    log.info("BOLLINGER BAND BREAKOUT BACKTEST RESULTS")
    log.info(f"{'='*70}")
...
def main():
    ...
    log.info(f"\nBacktesting Bollinger Breakout on {len(frames)} tickers...")
    all_trades, active = [], 0
