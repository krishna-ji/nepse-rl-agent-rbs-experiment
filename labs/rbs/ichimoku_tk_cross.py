#!/usr/bin/env python3
"""
NEPSE Ichimoku Tenkan/Kijun Cross Strategy — Rule-Based Backtest
================================================================
Rule-based trading system implementing the T/K Cross strategy
from "How to Make Money Trading the Ichimoku System" by B.M. Sadekar (Ch.5).

Signal: Tenkan-sen crosses above Kijun-sen (bullish) or below (bearish).
Cross strength classified by location relative to the Kumo:
  - Above Kumo  → strongly bullish / weakly bearish
  - Inside Kumo → neutral
  - Below Kumo  → weakly bullish  / strongly bearish

Entry Checklist (ALL must confirm for Long):
  1. T/K bullish cross is active (Tenkan > Kijun since the cross event)
  2. Future Kumo is bullish (Senkou A > Senkou B, unshifted)
  3. Chikou Span is free and clear above price from 26 periods ago
  4. Price > both Tenkan-sen and Kijun-sen (not over-extended)
  NOTE: Price vs. Kumo is NOT a hard requirement for T/K Cross
        (author gives it "less importance"). Cross strength is recorded instead.
...
Key Nuance from the Book:
  "There was a considerable lag between the T/K cross and the short entry
   point. What was the main reason for this delay? The Chikou."
  → The signal flag persists while Tenkan > Kijun, allowing delayed entry
    once Chikou clears. This patience is modelled faithfully here.
...
def setup():
    ...
    log = logging.getLogger("ichimoku_tk_cross")
    log.setLevel(logging.INFO)
    log.handlers.clear()
    fmt = logging.Formatter("%(asctime)s  %(message)s", datefmt="%H:%M:%S")
    fh = logging.FileHandler(RUN_DIR / "ichimoku_tk_cross.log", encoding="utf-8")
    fh.setFormatter(fmt); log.addHandler(fh)
    sh = logging.StreamHandler(sys.stdout)
    sh.setFormatter(fmt); log.addHandler(sh)

    log.info("=" * 70)
    log.info("NEPSE Ichimoku T/K Cross Strategy — Rule-Based Backtest")
    log.info("=" * 70)
...
# ============================================================================
# CROSS STRENGTH CLASSIFICATION
# ============================================================================

def classify_cross_strength(close_t, kumo_top_t, kumo_bot_t, direction):
    """
    Classify T/K cross strength by location relative to the Kumo.
...
def backtest_ticker(ticker, df):
    """
    Run T/K Cross strategy on a single ticker.

    State machine:
      FLAT → (T/K cross + conditions) → PENDING → (stop order fills) → POSITION → (exit) → FLAT

    After each trade exit, a new T/K cross event is required before re-entry.
    The cross signal persists as long as Tenkan remains above/below Kijun,
    modelling the book's "wait for Chikou to clear" pattern.
    """
    o = df["Open"].values
    h = df["High"].values
    l = df["Low"].values
    ...
    state = "FLAT"
    direction = None
    tk_cross_long  = False       # T/K bullish cross is active
    tk_cross_short = False       # T/K bearish cross is active

    # Pending order state
    pend_level = 0.0
    pend_stop  = 0.0
    sig_bar    = 0
    cross_str  = "neutral"       # cross strength at signal time
    ...
    for t in range(ICHIMOKU_WARMUP, n):
        ...
        # ── T/K cross detection (runs every bar) ──────────────────────
        ...
        # Invalidate crosses if the relationship reverses
        if tenkan[t] <= kijun[t]:
            tk_cross_long = False
        if tenkan[t] >= kijun[t]:
        ...
        if state == "PENDING":
            ...
            if filled:
                ...
                state = "POSITION"
                tk_cross_long  = False                     # consumed
                tk_cross_short = False
                continue                                   # don't evaluate exit on entry bar

            # Timeout or cross reversal
        ...
        if state == "POSITION":
            ...
            if exit_px is not None:
                ...
                trades.append({
                    "ticker":         ticker,
                    "direction":      direction,
                    "cross_strength": trade_cross_str,
        ...
        # Long signal (T/K cross, no strict price-vs-Kumo requirement)
        if tk_cross_long:
            cond1 = fut_sa[t] > fut_sb[t]                                   # future Kumo bullish
            cond2 = chikou_is_free(c, h, l, t, "long",                      # Chikou free
                                   sa_arr=senkou_a, sb_arr=senkou_b)         # incl. hist Kumo
            cond3 = (c[t] > tenkan[t]) and (c[t] > kijun[t])               # price above both
            cond4 = (c[t] - kijun[t]) / max(atr[t], 1e-10) < MAX_KIJUN_DIST  # not stretched
            ...
            if cond1 and cond2 and cond3 and cond4 and cond5:
                ...
                sig_bar   = t
                direction = "LONG"
                cross_str = classify_cross_strength(c[t], kumo_top[t],
                                                     kumo_bot[t], "LONG")
                state     = "PENDING"

        # Short signal
        elif not LONG_ONLY and tk_cross_short:
            cond1 = fut_sa[t] < fut_sb[t]
            cond2 = chikou_is_free(c, h, l, t, "short",
                                   sa_arr=senkou_a, sb_arr=senkou_b)
            cond3 = (c[t] < tenkan[t]) and (c[t] < kijun[t])
            cond4 = (kijun[t] - c[t]) / max(atr[t], 1e-10) < MAX_KIJUN_DIST
...
def report_results(all_trades, log, run_dir):
    ...
    log.info(f"\n{'='*70}")
    log.info("T/K CROSS BACKTEST RESULTS")
    log.info(f"{'='*70}")
    ...
    # ── By cross strength ──
    log.info(f"\n{'─'*50}")
    log.info("BY CROSS STRENGTH:")
    for cs, grp in df.groupby("cross_strength"):
        wr = (grp["net_pnl_pct"] > 0).mean() * 100
        log.info(f"  {cs:10s}: {len(grp):5d} trades, "
...
def _pf(grp):
    """Profit factor helper for a group of trades."""
    w = grp.loc[grp["net_pnl_pct"] > 0, "net_pnl_pct"].sum()
    l = abs(grp.loc[grp["net_pnl_pct"] <= 0, "net_pnl_pct"].sum())
...
def main():
    ...
    log.info(f"\nBacktesting T/K Cross strategy on {len(frames)} tickers...")
    all_trades = []
    active = 0
