#!/usr/bin/env python3
"""
NEPSE MACD Signal Line Cross Strategy — Rule-Based Backtest
===========================================================
Trend-following momentum system using MACD histogram direction changes.

Entry Logic (Long):
  1. MACD line crosses above Signal line                         — bullish cross
  2. MACD line is below zero at time of cross                    — buying the turn
  3. Histogram turns positive (confirmation)
  4. Price > SMA(50)                                             — trend filter
  5. MACD histogram increasing for at least 2 bars               — momentum

Entry Execution:
  Buy-stop at day's high (confirms momentum continuation).
  If not filled within ORDER_TIMEOUT bars, cancel.

Stop / Exit:
  - Initial stop: SMA(50) − ATR_STOP_MULT × ATR
  - Trail: SMA(50) − ATR buffer (ratchets up only)
  - Exit: MACD crosses below Signal (bearish cross)
  - Hard stop if price gaps below

Lookahead Bias Prevention:
  - MACD uses EMAs computed up to bar t
  - Signal at close of t → order fills on bar t+1
"""
...
def setup():
    ...
    log = logging.getLogger("macd_cross")
    log.setLevel(logging.INFO)
    log.handlers.clear()
    fmt = logging.Formatter("%(asctime)s  %(message)s", datefmt="%H:%M:%S")
    fh = logging.FileHandler(RUN_DIR / "macd_cross.log", encoding="utf-8")
    fh.setFormatter(fmt); log.addHandler(fh)
    sh = logging.StreamHandler(sys.stdout)
    sh.setFormatter(fmt); log.addHandler(sh)

    log.info("=" * 70)
    log.info("NEPSE MACD Signal Cross Strategy — Rule-Based Backtest")
    log.info("=" * 70)
    log.info(f"Run directory  : {RUN_DIR.resolve()}")
    log.info(f"MACD           : {FAST_EMA}/{SLOW_EMA}/{SIGNAL_EMA}")
    log.info(f"Trend SMA      : {TREND_SMA}")
...
def compute_indicators(df):
    ...
    fast = c.ewm(span=FAST_EMA, adjust=False).mean()
    slow = c.ewm(span=SLOW_EMA, adjust=False).mean()
    macd_line = fast - slow
    signal_line = macd_line.ewm(span=SIGNAL_EMA, adjust=False).mean()
    histogram = macd_line - signal_line

    trend_sma = c.rolling(TREND_SMA).mean()
...
def backtest_ticker(ticker, df):
    ...
    for t in range(WARMUP, n):
        if any(np.isnan(x[t]) for x in [macd, signal, hist, tsma, atr]):
            continue
        ...
        if state == "PENDING":
            ...
            if direction == "LONG":
                cancel = cancel or macd[t] < signal[t]
            if cancel:
        ...
        if state == "POSITION":
            ...
            if direction == "LONG":
                ...
                elif macd[t] < signal[t] and t > 0 and macd[t-1] >= signal[t-1]:
                    exit_px = c[t]; exit_rsn = "macd_cross"
                elif c[t] < tsma[t]:
                    exit_px = c[t]; exit_rsn = "trend_close"
                else:
                    trail = tsma[t] - ATR_STOP_MULT * atr[t]
        ...
        # ── FLAT — scan for MACD bullish cross ────────────────────────
        if t > 1:
            cross = macd[t] > signal[t] and macd[t-1] <= signal[t-1]  # cross up
            below_zero = macd[t] < 0                                   # buying the turn
            above_trend = c[t] > tsma[t]                               # trend filter
            hist_rising = hist[t] > hist[t-1] and hist[t-1] > hist[t-2] if t > 1 else False

            if cross and below_zero and above_trend and hist_rising:
                pend_level = h[t]
                pend_stop  = tsma[t] - ATR_STOP_MULT * atr[t]
                sig_bar    = t
                direction  = "LONG"
                state      = "PENDING"
...
def report_results(all_trades, log, run_dir):
    ...
    log.info(f"\n{'='*70}")
    log.info("MACD SIGNAL CROSS BACKTEST RESULTS")
    log.info(f"{'='*70}")
...
def main():
    ...
    log.info(f"\nBacktesting MACD Cross on {len(frames)} tickers...")
    all_trades, active = [], 0
